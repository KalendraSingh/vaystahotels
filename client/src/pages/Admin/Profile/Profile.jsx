...
function Profile() {
  ...
  const adminId = adminAuth.data && adminAuth.data.id;
  ...
  const updateVendorProfileData = async () => {
    ...
    try {
      const res = await updateAdminProfile(formData, adminId);
      if (res.status === 200) {
                ...
                {toggleOptions.map((option, index) => (
                  <div key={index} className='flex items-center gap-4'>
                    <label className='relative inline-flex items-center cursor-pointer'>
                      ...
                      />
                      <div className='w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:bg-orange-600'>
                        <div
